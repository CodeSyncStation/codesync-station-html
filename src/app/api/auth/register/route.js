...
export async function POST(request) {
  ...
  try {
    ...
    if (error.code === 11000) { // Handle duplicate email error
      return NextResponse.json({ status: 400, message: "Email already exists" }, { status: 400 });
    }

    console.error("Error creating user:", error);
    return NextResponse.json({ status: 500, message: "Something went wrong, please try again later." }, { status: 500 });
  }
}
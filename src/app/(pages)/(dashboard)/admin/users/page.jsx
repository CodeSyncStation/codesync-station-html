...
import { useDeleteUserMutation, useGetAllUsersQuery } from "@/redux/api/user/userSlice";
...
const page = () => {
  ...
  const { isLoading, isError, error, data: moderators } = useGetAllUsersQuery({ page, isMod: true })
  ...
  const handleDelete = () => {
    const promise = deleteUser({ iri }).unwrap()
    .then(res=> {
      showWarning(false)
    })
    toast.promise(promise, {
      ...
      error: (err)=> err["hydra:title"] || "Un expected error!",
    })
  }
  ...
  } else if (!isLoading && !isError && moderators["hydra:member"]?.length === 0) {
    content = <td colSpan="4" className=" fw-bold text-center bg-white">There is No Moderator!</td>
  } else if (!isLoading && !isError && moderators["hydra:member"]?.length > 0) {
    content = moderators["hydra:member"].map(mod => {
      const { fullName, phone, email, roles, registeredAt, course, profileImage, social, occupation, bio, isMod, takenCourses } = mod;
      return (
        <tr key={mod["@id"]}>
          <td>
          ...
          <td>
            <div className="d-flex gap-2">
              <button onClick={() => handleEdit(mod["@id"])} className="pill  text-black" style={{ "background-color": "#E2E8F0" }}>
                <span className="icon">
  ...
  return (
    ...
    <>
      ...
      <WarningModal handleConfirm={handleDelete} show={warning} setShow={showWarning} title="Delete Moderator">
        Are you still sure you want to delete the <strong>Diving Deep into Color Grading and Correction</strong> user? All the details including user media and students will be deleted from the database.
      </WarningModal>
      <section className="moderator-container">
